...
<script type="text/javascript">
  window.onload=function(){
      document.getElementById("op37").style.color = 'white';
    }
  $(function () {
...
Highcharts.chart('container', {
  <?php
    while($row = pg_fetch_array($datos)){
      $cantidad[] = $row['tdato']; 
      $descripcion[] = $row['ac_descripcion']; 
  ...
  title: {
    text: 'Casos de violencia contra las mujeres registrados e informados, según tipo de registro, en la provincia de Tucumán. Período: <?php echo date("d/m/Y", strtotime($_POST['fecha1'])); ?> - <?php echo date("d/m/Y", strtotime($_POST['fecha2'])); ?>.',
   style: { 
  	     fontSize: '14px', 
  ...
  xAxis: {
    categories: [<?php $filas = pg_num_rows($datos); for($i=0;$i<$filas;$i++){echo '\''.$descripcion[$i].'\''.',';} ?>],
    title: {
      text: null
    }
  },
            ...
            <div class="card-body">
              <form id="enviar" name="enviar" role="form" class="form-horizontal" enctype="multipart/form-data"  method="post" action="">
                  ...
                  <div class="form-row">
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                      <div class="form-label-group">
                        <b>Accesibilidad al servicio <?php if($_POST){ echo 'desde '.date("d/m/Y", strtotime($_POST['fecha1'])).' hasta '.date("d/m/Y", strtotime($_POST['fecha2'])); } ?></b>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="table-responsive">
                          <table id="tablaPorAccesibilidad" name="tabla" class="table table-bordered col-xs-12 col-sm-12 col-md-12 col-lg-12">
                            <thead>
   ...
   <script type="text/javascript">
      $('#buscar').on('click', function(event) {
        ...
        if (fecha1 == '') {
          ...
          $.ajax({
            url: '<?php echo URL; ?>poraccesibilidad/por_accesibilidad',
            type: 'POST',
            dataType: 'html',
    ...
    $('#tablaPorAccesibilidad').DataTable( {
        dom: 'Bfrtip',
        "bFilter": false,
        language: {
    ...
    {
      extend: 'excelHtml5',
      title: 'Casos de violencia contra las mujeres registrados e informados, según tipo de registro, en la provincia de Tucumán. Período: <?php echo date("d/m/Y", strtotime($_POST['fecha1'])); ?> - <?php echo date("d/m/Y", strtotime($_POST['fecha2'])); ?>.',
      className: 'btn',
      text: "Excel"
  },
  {
...
{
  extend: 'print',
  title: '<H2>Casos de violencia contra las mujeres registrados e informados, según tipo de registro, en la provincia de Tucumán. Período: <?php echo date("d/m/Y", strtotime($_POST['fecha1'])); ?> - <?php echo date("d/m/Y", strtotime($_POST['fecha2'])); ?>.</H2><H5>Registro Único de Casos de Violencia contra las Mujeres (RUCVM)</H5><BR/>',
  className: 'btn',
  text: "Imprimir"
},
 {
