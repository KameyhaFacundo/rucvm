<?php
	...
	if($idprofesional != NULL){
		$sql = "SELECT * FROM profesional, usuarios where usuarios.idinstitucion = profesional.idinstitucion AND usuarios.idusuario = $idusuario AND idprofesional<>$idprofesional ORDER BY idprofesional";
	} else {
	...
	echo '<option value="NULL">Seleccionar profesional</option>';
	if(isset($row2)){
	...
	{ 
		echo '<option value="'.$row['idprofesional'].'">'.ucwords(strtolower($row['pro_apellido'])).", ".ucwords(strtolower($row['pro_nombre'])).'</option>';         
	}