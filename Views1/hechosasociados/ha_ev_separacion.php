...
Highcharts.chart('container', {
  <?php
    while($row = pg_fetch_array($datos)){ 
        ...
        $separacion[] = $row['disp_descripcion'];
  ...
  title: {
    text: 'Casos de violencia contra las mujeres registrados e informados, según intención de separarse asociado a episodios violentos, en la provincia de Tucumán. Período: <?php echo date("d/m/Y", strtotime($_POST['fecha1'])); ?> - <?php echo date("d/m/Y", strtotime($_POST['fecha2'])); ?>.',
   style: { 
         fontSize: '14px', 
                  ...
                  <div class="form-row">
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                      <div class="form-label-group">
                        <b>Hechos asociados a episodios violentos: Intención de separarse <?php if($_POST){ echo 'desde '.date("d/m/Y", strtotime($_POST['fecha1'])).' hasta '.date("d/m/Y", strtotime($_POST['fecha2'])); } ?></b>
                      </div>
                    </div>
                  </div>
                </div>
                          ...
                          <table id="tablaOcuConsultante" name="tabla" class="table table-bordered col-xs-12 col-sm-12 col-md-12 col-lg-12">
                            <thead>
                              <tr style="background-color: #b3434375">  
                                <td><center><strong>Intención de separarse</strong></center></td>
                            ...
                            <tbody>
                              ...
                              <tr bgcolor="#DAD2D9">
                                <td><center><?php echo $separacion[$i]; ?></center></td>
   ...
   <script type="text/javascript">
      $('#buscar').on('click', function(event) {
        ...
        if (fecha1 == '') {
          ...
          $.ajax({
            url: '<?php echo URL; ?>hechosasociados/ha_ev_separacion',
            type: 'POST',
            dataType: 'html',
    ...
    {
      extend: 'excelHtml5',
      title: 'Casos de violencia contra las mujeres registrados e informados, según intención de separarse asociado a episodios violentos, en la provincia de Tucumán. Período: <?php echo date("d/m/Y", strtotime($_POST['fecha1'])); ?> - <?php echo date("d/m/Y", strtotime($_POST['fecha2'])); ?>.',
      className: 'btn',
      text: "Excel"
  },
  {
...
{
  extend: 'print',
  title: '<H2>Casos de violencia contra las mujeres registrados e informados, según intención de separarse asociado a episodios violentos, en la provincia de Tucumán. Período: <?php echo date("d/m/Y", strtotime($_POST['fecha1'])); ?> - <?php echo date("d/m/Y", strtotime($_POST['fecha2'])); ?>.</H2><H5>Registro Único de Casos de Violencia contra las Mujeres (RUCVM)</H5><BR/>',
  className: 'btn',
  text: "Imprimir"
},
 {
