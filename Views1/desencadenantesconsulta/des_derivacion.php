...
Highcharts.chart('container', {
  <?php
    while($row = pg_fetch_array($datos)){ 
        ...
        $derivacion[] = $row['disp_descripcion'];
  ...
  title: {
    text: 'Casos de violencia contra las mujeres registrados e informados, según la derivación como desencadenante de consulta por parte de la consultante, en la provincia de Tucumán. Período: <?php echo date("d/m/Y", strtotime($_POST['fecha1'])); ?> - <?php echo date("d/m/Y", strtotime($_POST['fecha2'])); ?>.',
   style: { 
         fontSize: '14px', 
                  ...
                  <div class="form-row">
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                      <div class="form-label-group">
                        <b>Desencadenantes de la consulta: Derivación <?php if($_POST){ echo 'desde '.date("d/m/Y", strtotime($_POST['fecha1'])).' hasta '.date("d/m/Y", strtotime($_POST['fecha2'])); } ?></b>
                      </div>
                    </div>
                  </div>
                </div>
                          ...
                          <table id="tablaOcuConsultante" name="tabla" class="table table-bordered col-xs-12 col-sm-12 col-md-12 col-lg-12">
                            <thead>
                              <tr style="background-color: #b3434375">  
                                <td><center><strong>Derivación</strong></center></td>
                            ...
                            <tbody>
                              ...
                              <tr bgcolor="#DAD2D9">
                                <td><center><?php echo $derivacion[$i]; ?></center></td>
   ...
   <script type="text/javascript">
      $('#buscar').on('click', function(event) {
        ...
        if (fecha1 == '') {
          ...
          $.ajax({
            url: '<?php echo URL; ?>desencadenantesconsulta/des_derivacion',
            type: 'POST',
            dataType: 'html',
    ...
    {
      extend: 'excelHtml5',
      title: 'Casos de violencia contra las mujeres registrados e informados, según la derivación como desencadenante de consulta por parte de la consultante, en la provincia de Tucumán. Período: <?php echo date("d/m/Y", strtotime($_POST['fecha1'])); ?> - <?php echo date("d/m/Y", strtotime($_POST['fecha2'])); ?>.',
      className: 'btn',
      text: "Excel"
  },
  {
